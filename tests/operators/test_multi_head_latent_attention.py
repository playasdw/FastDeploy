...
if "fastdeploy.model_executor.ops.iluvatar" not in sys.modules:
    ...
    sys.modules["fastdeploy.model_executor.ops.iluvatar"] = types.ModuleType("fastdeploy.model_executor.ops.iluvatar")
...
class TestMultiHeadLatentAttention(unittest.TestCase):
    ...
    def _build_inputs(self, dtype: str, max_dec_len: int = 0):
        query = paddle.zeros([self.token_num, self.q_num_heads * self.head_dim_qk], dtype=dtype, place=self.place)
        ...
        block_tables = paddle.zeros([self.batch_size, self.max_blocks_per_seq], dtype="int32", place=self.place)
